  ...
  <div id="app">
    <div class="container">
      ...
      <table class="table table-bordered table-hover mt-2">
        ...
        <tbody>
          <tr v-for="(t, n) in list" :key="t.id">
            <td>{{ t.id }}</td>
            <td>{{ t.name }}</td>

            <!-- 如果价格超过100，就有red这个类 -->
            <td :class="{ red: t.price > 100 }">{{ t.price }}</td>
            <td>{{ t.time | gett }}</td>
            <td><a @click.prevent="dels(n)" href="#">删除</a></td>
          </tr>
      ...
      <form class="form-inline">
        ...
        &nbsp;&nbsp;&nbsp;&nbsp;
        <!-- 阻止表单提交 -->
        <button @click.prevent="ha" class="btn btn-primary">添加资产</button>
      </form>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    allPrice() {
      return Math.floor(
        this.list.reduce((sum1, sum2) => (sum1 += sum2.price), 0)
      );
    },
    svgPrice() {
  ...
  watch: {
    list: {
      ...
      immediate: true,
      handler(newList) {
        localStorage.setItem("datas", JSON.stringify(newList));
